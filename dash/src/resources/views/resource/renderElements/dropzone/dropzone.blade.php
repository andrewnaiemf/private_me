...
<div class="col-{{ $field['column'] }}" id="drop_{{ $dz_param }}">
...
@endphp
    var i=0;
        ...
        @if(preg_match('/image/i',$file->mimtype))
         myDropzone{{$dz_param}}.emit('thumbnail', mockFile, '{{ Storage::url($file->full_path) }}');
        ...
        .attr("url",'{{ Storage::url($file->full_path) }}')
  ...
  // On Delete File must be remove from server also
   myDropzone{{$dz_param}}.on("removedfile", function(file) {
    ...
    if(file_id !== undefined && file_id !== undefined && file_type !== undefined){
      //Ajax Delete Request
      $.ajax({
        url:'{{ url($route.'/delete/file') }}',
        dataType:'json',
        type:'post',
        data:{_token:'{{ csrf_token() }}',file_id:file_id,file_type:file_type,id:file_id}
      });
    }
  });
